...
public class GrabFansInfo {
	public static Logger logger = Logger.getLogger(GrabFansInfo.class);
	private GrabPageResource grabPageSource = null;
	private UrlPojo urlPojo;
	private String url;
	private UrlsManager urlsManager;
	@Autowired
	private Personfans personfans;
	@Autowired
	...
	String temp_fans_url = "";
	String temp_fans_list = "";
	// StringBuilder temp_attention_list=null;
	String relationInfo = "";

	String domId = null;
	AnsjPaser innerUlParser = new AnsjPaser("<ul class=\"cnfList\"", "</ul>",
			AnsjPaser.TEXTEGEXANDNRT);

	public String savePersonFansInfo() throws Exception {
		...
		while (i <= StaticValue.grab_attention_max_pageNum) {
			 ...
			 */
			System.out.println("正在抓取" + this.url + "粉丝第 " + i + " 页");
			if (i == 1) {
				temp_fans_url=StaticValue.fans_req_url_first.replace("${domain_id+uid}",
						this.domain_id + this.uid).replace("${domain_id}",
								this.domain_id);
			} else {
				temp_fans_url = StaticValue.fans_req_url.replace(
						"${domain_id+uid}", this.domain_id + this.uid).replace(
						"${pageNo}", "" + i).replace("${pids}", domId);
			}

			// logger.info("正在抓取的url---------------" + temp_attention_url);
			content = grabPageSource.getPageSourceOfSina(temp_fans_url, null);
			...
			if (relationInfo != null) {
				// System.out.print(temp_fans_url + "   ");
				...
				if (i == 1) {
					...
					if (!StringOperatorUtil
						...
						temp_fans_list = parsePageResoure(selectorResultList
								.get(0));
					}
				} else {
					...
					if (!StringOperatorUtil
						...
						// IOUtil
						// .writeFile("d:/test.txt", selectorResultList.get(0));
						temp_fans_url = parsePageResoure(selectorResultList
								.get(0));

						if (temp_fans_url != null) {
							if (temp_fans_list.contains(temp_fans_url
									.split("#")[0])) {
								logger.info("关注列表已抓到头了，共抓取了" + (i - 1) + "页");
								break;
							} else {
								temp_fans_list += ("#" + temp_fans_url);
							}
						}
					} else {
						// 没抓取到任何记录,认为不能抓或没有了!
						System.out.println(this.url + "粉丝的第 " + i + " 页"
								+ ",没有抓取到记录，认为该url粉丝抓取结束");
						break;
					}
				}
				System.out.println("完成抓取" + this.url + "粉丝第 " + i + " 页");
				i++;
		...
		this.personfans = new Personfans();
		this.personfans.setUid(uid);
		this.personfans.setSendUrl(this.url);
		this.personfans.setFansIdList(temp_fans_list);
		// logger.info("attention list --" + temp_attention_list);
		String[] attention_array = temp_fans_list.split("#");
		if (temp_fans_list.trim().length() == 0) {
			personfans.setFansSize(0);
		} else {
			personfans.setFansSize(attention_array.length);
			logger.info(url + "粉丝列表抓取完毕,抓取到的fans_size的大小是--------"
					+ attention_array.length);
			logger.info("即将抓取" + url + "粉丝列表的微博用的个人信息");
			// this.urlsManager.addUrlsArray(attention_array, this.url);// 暂注掉
		}
		if (this.personAttentionManager.saveOrUpdateFans(personfans)) {
			logger.info(this.url + "-----保存粉丝列表信息成功");
		} else {
			logger.info(url + "的粉丝列表保存出现问题，无法抓取关注用户的个人信息了!");
		}
		return temp_fans_list;
	}

//	public static String beginRegex = "<li" + AnsjPaser.TEXTEGEXANDNRT
//			+ "action-data=\"uid=";
