package com.weibo.sina.cookie.government;
...
public class GrabGovContentInfo {
	public static Logger logger = Logger.getLogger(GrabGovContentInfo.class);
	@Autowired
	...
	public void grabArticleContents(UrlPojo urlPojo) throws Exception {
		...
		while (page <= 50) {
			...
			logger.info("政府版---正在抓取" + this.current_URL + "的第" + page + "页");
			if (page == 1) {
				// 先抓取第一页
		...
		// logger.info("政府版---" + this.sendUrl + "抓取结束");
		System.out.println("gov info grab end!");
	}
	...
	public boolean grabOnePageArticle(String url, boolean isFirstPage) {
		...
		if (isFirstPage) {
			...
			// logger.info("government content----" + content);
		} else {
	...
	public boolean saveOnePageContent(String content) {
		...
		// endRegex = "/dl>[\\s]*<dl";
		endRegex = "<dl action-type=\"feed_list_item\"";
		...
		while (list_parser.hasNext()) {
			...
			// logger.info("temp_item--" + temp_item);
			...
			try {
				...
				e.printStackTrace();
				logger.info("政府版--微博内容抓取时出现问题，此url抓取结束");
				return false;
			}
			// logger.info("政府版doc--" + doc);
			System.out.println("gov doc save successful");
		}
		repeat_count = 0;
