...
public class GrabProContentInfo {
	public static Logger logger = Logger.getLogger(GrabProContentInfo.class);
	@Autowired
	...
	public void grabArticleContents(UrlPojo urlPojo) throws Exception {
		...
		while (page <= 50) {
			...
			// logger.info("专业版---正在抓取" + this.current_URL + "的第" + page + "页");
			if (page == 1) {
				// 先抓取第一页
		...
		// logger.info("专业版---" + this.sendUrl + "抓取结束");
	}
	...
	public boolean saveOnePageContent(String content) {
		...
		while (list_parser.hasNext()) {
			...
			if (content_temp == null || content_temp.trim().length() == 0) {
				...
				doc.setDocUrl(FormatOutput.urlPrefix_ProAndEnter + doc.getPersonId() + "/"
						+ content_temp);
			}
			...
			try {
				...
				e.printStackTrace();
//				logger.info("专业版--微博内容抓取时出现问题，此url抓取结束");
				return false;
			}
//			logger.info("专业版doc--" + doc);
			System.out.println("professional save successful!");
		}
		repeat_count = 0;
